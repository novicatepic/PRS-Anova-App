...
public class InputParameters extends JFrame {
	...
	private double tempContrastValue;
	private final double alfaForContrast = 0.9500;
	private double tabulatedF;
	private double calculateTempContrastValue() {
		int otherParameter = numAlternatives * (numMeasurements - 1);
		
		double t = 0.0;
		TDistribution tDistribution = new TDistribution(otherParameter);
		while(alfaForContrast > tDistribution.cumulativeProbability(t)) {
			t += 0.001;
		}
		return t;
	}
	
	private double calculateTabulatedF() {
		double t = 0.0;
		int firstParam = degFreedomAlternatives;
		int secondParam = degFreedomErrors;
		FDistribution tDistribution = new FDistribution(firstParam, secondParam);
		
		while(0.95 > tDistribution.cumulativeProbability(t)) {
			t += 0.001;
		}
	...
	public void setParameters(int numAlternatives, int numMeasurements) {
		
		this.numAlternatives = numAlternatives;
		this.numMeasurements = numMeasurements;
		tempContrastValue = calculateTempContrastValue();
	...
	private void calculateParameters() {
		...
		tabulatedF = calculateTabulatedF();
		System.out.println("TABULATED F  IS " + tabulatedF);
		if(computedF > fValue) {
		...
		ShowResults showResults = new ShowResults(meanSquareColumns, effects, meanSquare, SSA, SSE, SST, degFreedomAlternatives, degFreedomErrors,
				degFreedomTotal, meanSquareAlternatives, meanSquareErrors, computedF, tabulatedF);
	...
	private void calculateContrast(int alternative1, int alternative2) {	
		...
		double c1 = c - tempContrastValue * sc;
		double c2 = c + tempContrastValue * sc;
